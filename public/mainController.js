...
app.controller('Contact',function($scope,$http){
  ...
  var feedback={firstname:$scope.firstname,lastname:$scope.lastname,email:$scope.email,message:$scope.message,phone:$scope.phone}
    jQuery.post("sendMail",{feedback:feedback} ,function(result){
...
app.controller('myAc', function($scope,$http,$location){
  ...
  names.forEach(element => {
      var element="#"+element;
   output.push({name:element,value:jQuery(element).val()});
  });
...
if (!Customer.currentRoom) {
    if (Customer.rooms) {
        ...
        var Rm={} 
        Rm._id = "598eee92fbbace03d0d68746";Rm. name="Kids Room";
        Rm. price =14;
        Rm. beds = 1,
        Rm. status ="no";
        Rm.rating = 4;
...
var diff=dhm(time);
  ...
  $scope.total= $scope.currentRoom.price * $scope.nights
   ...
   if (diff<0) {
    ...
    toastr.error('Error', 'invalid date', {timeOut: 5000});
    $scope.from=new Date();
    var day = new Date();
    console.log(day); 
...
document.getElementById('datepicker4').addEventListener("change",function(){
 ...
 $scope.ser=function(service,amount){
    var dbd=null;
    dbd=$scope.services;   
 var index=   _.findIndex(dbd,{service:service});
 ...
 if (index!== -1) {
  ...
  //  dbd[index]={service:service,amount:amount};
    $scope.services=dbd;
     ...
     dbd.push({service:service,amount:amount*$scope.nights})
     dbd=_.reverse(dbd);
    $scope.services=dbd;
   
 }

var totalex= _.sumBy($scope.services, function(o) { return o.amount; });
    ...
    Stripe.card.createToken({
        ...
        exp_year: jQuery('#year_c').val()
          ...
          if (response.error) {
            ...
            var booking={from:$scope.from,to:$scope.to,roomId:$scope.currentRoom._id,userId:$scope.userId,amount:$scope.total,expenses:$scope.services,paid:0}
           	
	
            toastr.success('Processing', 'submitting', {timeOut: 5000});
            jQuery.post("token",{token:token,book:booking,mail:$scope.generateMail(),email:$scope.email} ,function(result){
                if (result=="charged") {
...
$scope.goto=function(){
  ...
  $scope.next=function(){
    ...
    var user={firstname:$scope.firstname, lastname:$scope.lastname,email:$scope.email,number:$scope.number,withpet:jQuery('#with_pet_select').val(),parking:jQuery('#parking_select').val(),request:$scope.specialRequest}
