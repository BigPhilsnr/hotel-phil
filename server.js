...
//this get the detail for the home page
  ...
  exec(function(err,news){
    ...
    res.send({news:news,hf:hf,feedback:feedback,banner_title1:item.line1,banner_title2:item.line2,banner_src:item.src,welcome_info:I2Item,rooms:rooms});
   })
...
app.post("/loginx",function(req,res){
  ...
  Employee.findOne({email:email},function(err,em){
    if (err) {
      console.log(err.message)
        res.send({success:0,user:err.message});
    } else {
  ...
  bcrypt.compare(password, em.password, function(err, correct) {
      ...
      if (correct==true) {
          em.password=null;;
       console.log({success:1,user:em});
     //  res.send(student)
      res.send({success:1,user:em});
   } else {
       res.send({success:0,user:"wrong username or email"});
   }
...
if (username=="admin" && password=="admin123") {
  ...
  console.log("romm are there")
   res.render('admin.ejs',{rooms:rms})
  ...
  room.save(function(err){
    Room.find((err, rooms)=>{
      if(err)return console.error(err);
       ...
       res.render('admin.ejs',{rooms:rooms})
...
if (req.body.id) {
 ...
 Room.update({ _id: id }, { $set: req.body}, function(err,update){
 if (err) {
...
app.post('/hf', function (req, res, next) {
  console.log(req.body)
  Itab.update({_id:"5aee68fe134c1630ccb04865"},{$set:req.body},function(err,rez){
...
stripe.charges.create({
  ...
  new Booking(book).save(function(err,booking){
    if (err) {
      ...
      Room.update({ _id: req.body.roomId }, { $set: {status:'no'}}, function(err,update){
        if (err) {
...
console.log(req.body)
  new User(req.body).save(function(err,result){
    if (err) {
     res.send({success:0,message:err.name}) 
    }else{
      console.log(result);
      res.send({success:1,id:result._id})
    }
  })
...
delete req.body["password2"]
  if (req.body._id) {
    id=req.body._id;
    delete req.body["_id"]
    Employee.update(id,{$set:req.body},function(err,result){
      if (err) {
        console.log(err.message);
...
Booking.update({ _id: id }, { $set: req.body}, function(err,update){
 if (err) {
  ...
  Room.update({ _id: req.body.roomId }, { $set: {status:status}}, function(err,update){
    if (err) {
...
Booking.update({ roomId: rid }, { $set: {paid:0}}, function(err,update){
 if (err) {
   
   console.log(err.message)
 }else{
   
  Room.update({ _id: rid }, { $set: {status:'yes'}}, function(err,update){
    if (err) {
...
app.post("/updateUser",(req,res)=>{
 ...
 User.update({ _id: id }, { $set: req.body.user}, function(err,update){
  if (err) {
